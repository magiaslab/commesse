...
export function CommessaInvoicesTable({ commessaId }: { commessaId: number }) {
  ...
  useEffect(() => { load(); }, [commessaId, q, page, pageSize, showDeleted, status, dateFrom, dateTo]);

  const exportHref = (() => {
    const params = new URLSearchParams({
      commessaId: String(commessaId), q, includeDeleted: String(showDeleted), format: 'csv'
    });
    if (status && status !== 'all') params.set('statoPagamento', status);
    if (dateFrom) params.set('dateFrom', dateFrom);
    if (dateTo) params.set('dateTo', dateTo);
          ...
          rightSlot={
            <div className="flex items-center gap-2">
              ...
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={showDeleted} onCheckedChange={(v)=> { setPage(1); setShowDeleted(Boolean(v)); }} /> Mostra eliminati
              </label>
            </div>
          }
                ...
                <TableRow key={f.id} className={f.statoPagamento === 'DELETED' ? 'bg-muted/30 text-muted-foreground' : undefined}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <DeleteInvoiceButton id={f.id} onDeleted={()=> setRows((arr)=> arr.filter((x)=> x.id!==f.id))} />
                    </div>
                  </TableCell>
                </TableRow>
              ))}
...
function DeleteInvoiceButton({ id, onDeleted }: { id: number; onDeleted: ()=>void }) {
  const handleDelete = async () => {
    ...
    if (r.ok) {
      onDeleted();
      toast.success('Fattura eliminata', { action: { label: 'Annulla', onClick: async () => { await fetch(`/api/invoices/${id}`, { method: 'POST' }); } } });
    } else {
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>Soft delete del record.</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
