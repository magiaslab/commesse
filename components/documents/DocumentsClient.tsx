...
export function DocumentsClient({ initial }: { initial: any }) {
  const [docs, setDocs] = useState<any[]>(initial.items || []);
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const params = new URLSearchParams({ q, page: String(page), pageSize: String(pageSize), includeDeleted: String(showDeleted) });
        if (commessaId) params.set('commessaId', commessaId);
        if (supplierId) params.set('supplierId', supplierId);
        const res = await fetch(`/api/documents?${params.toString()}`, { cache: 'no-store', credentials: 'include' });
  ...
  }, [q, page, pageSize, showDeleted, commessaId, supplierId]);

  return (
    <main className="p-6">
      <div className="grid gap-2 md:grid-cols-4 mb-2">
        <input className="h-9 w-full rounded-md border px-3 text-sm" placeholder="Filtra per ID commessa" value={commessaId} onChange={(e)=> { setPage(1); setCommessaId(e.target.value); }} />
        <input className="h-9 w-full rounded-md border px-3 text-sm" placeholder="Filtra per ID fornitore" value={supplierId} onChange={(e)=> { setPage(1); setSupplierId(e.target.value); }} />
      </div>
      <TableToolbar
        query={q}
        ...
        exportHref={`/api/documents?q=${encodeURIComponent(q)}&includeDeleted=${showDeleted}&format=csv${commessaId?`&commessaId=${commessaId}`:''}${supplierId?`&supplierId=${supplierId}`:''}`}
