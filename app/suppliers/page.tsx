...
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ChevronsUpDown, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function SuppliersPage() {
  ...
  const columns = useMemo<ColumnDef<Supplier>[]>(
    () => [
      { header: () => <div className="inline-flex items-center gap-1">ID</div>, accessorKey: 'id' },
      { header: () => <div className="inline-flex items-center gap-1">Ragione Sociale</div>, accessorKey: 'ragioneSociale' },
      { header: () => <div className="inline-flex items-center gap-1">Email</div>, accessorKey: 'email' },
      {
        header: 'Azioni',
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <Link href={`/suppliers/${row.original.id}`} className="text-primary hover:underline">
              Dettaglio
            </Link>
            <Link href={`/suppliers/${row.original.id}/edit`} className="text-gray-600 hover:underline">
              Modifica
            </Link>
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Eliminare questo fornitore?</AlertDialogTitle>
  ...
  return (
    <main className="p-6">
      <TableToolbar
        ...
        exportHref={`/api/suppliers?format=csv&q=${encodeURIComponent(globalFilter)}`}
        info={data.length ? `${(page-1)*pageSize + 1}–${Math.min(page*pageSize, total)} di ${total}` : '0 risultati'}
        rightSlot={<Button asChild><Link href="/suppliers/new">Nuovo fornitore</Link></Button>}
        ...
        <div className="rounded-md border overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-2 p-3 border-t bg-background">
            <div className="text-sm text-muted-foreground">
              {data.length ? `${(page-1)*pageSize + 1}–${Math.min(page*pageSize, total)} di ${total}` : 'Nessun fornitore'}
            </div>
            <div className="flex items-center gap-2">
